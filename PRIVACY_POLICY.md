...
Media requests are made to the original site or media host selected by the user. The extension does not upload media or browsing data to an Aura server. Downloads are written to a user-selected local folder; when the offscreen worker cannot write directly, a single media file may be saved through the browser's default download feature. The extension package contains no fixed extension key, private native-host bridge, or remote code.
