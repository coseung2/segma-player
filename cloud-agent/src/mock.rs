    ...
    F: FnMut(&CloudJobState) -> io::Result<()>,
{
    ...
    let file_name = request
        ...
        .ok_or_else(|| {
            io::Error::new(io::ErrorKind::InvalidInput, "upload file name is invalid")
        })?;
    ...
    aura_companion_contract::write_json_atomic(&manifest_path(root, &request.item_id), &manifest)?;
    ...
    F: FnMut(&CloudJobState) -> io::Result<()>,
{
    ...
    let destination = request
        ...
        .map(Path::new)
        .ok_or_else(|| {
            io::Error::new(io::ErrorKind::InvalidInput, "download requires localPath")
        })?;
    if destination.exists() {
        return Err(io::Error::new(
...
mod tests {
    ...
    fn download_refuses_to_replace_an_existing_file() {
        ...
        let mut download_state = CloudJobState::queued(&download, 2);
        let error =
            execute_with_chunk_size(&root, &download, &mut download_state, 4, &mut |_| Ok(()))
