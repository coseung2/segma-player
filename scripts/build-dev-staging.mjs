...
function validateCompanionInstallUrl(value) {
  if (!value) return "";
  const url = new URL(value);
  if (url.protocol !== "https:" || url.username || url.password) {
    throw new Error("Companion install URL must be absolute HTTPS without embedded credentials.");
  }
  return url.href;
}
...
async function writeEdition(stageDirectory, edition, upgradeUrl, companionInstallUrl) {
  ...
  await writeFile(path.join(stageDirectory, "edition.js"), [
    ...
    `export const COMPANION_INSTALL_URL = ${JSON.stringify(companionInstallUrl)};`,
    "",
  ].join("\n"), "utf8");
}
...
export async function buildDevStaging({
  ...
  version = "",
  upgradeUrl = "",
  companionInstallUrl = "https://aura.mdownloader.workers.dev/download",
} = {}) {
  ...
  const selectedCompanionInstallUrl = validateCompanionInstallUrl(companionInstallUrl);
  ...
  await writeEdition(stageDirectory, selectedEdition, selectedUpgradeUrl, selectedCompanionInstallUrl);
