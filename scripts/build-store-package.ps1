...
param(
  ...
  [string]$UpgradeUrl = '',
  [string]$CompanionInstallUrl = 'https://aura.mdownloader.workers.dev/download',
  [string]$Version = '',
...
if ($CompanionInstallUrl) {
  $parsedCompanionInstallUrl = $null
  $validCompanionInstallUrl = [System.Uri]::TryCreate($CompanionInstallUrl, [System.UriKind]::Absolute, [ref]$parsedCompanionInstallUrl)
  if (-not $validCompanionInstallUrl -or $parsedCompanionInstallUrl.Scheme -ne 'https' -or $parsedCompanionInstallUrl.UserInfo) {
    throw 'Companion install URL must be an absolute HTTPS URL without embedded credentials.'
  }
}
...
function Write-Edition {
  ...
  $companionInstallLiteral = $CompanionInstallUrl | ConvertTo-Json -Compress
...
export const COMPANION_INSTALL_URL = $companionInstallLiteral;
"@
  } else {
    @"
...
if (-not $CompanionInstallUrl) {
  Write-Output 'WARNING=Companion install URL is empty; the install guidance link will remain hidden.'
}
