# Chrome Web Store listing — English

## Name

Aura Media Downloader

## Short description

Detect compatible media and save authorized downloads locally.

## Detailed description

Aura Media Downloader helps you identify compatible video and audio resources on pages you are allowed to use and save those resources locally. It is a single-purpose utility for media detection and user-authorized downloads.

Features:

- Detects compatible direct and streaming media resources on the current page.
- Shows a text-based candidate list with the page title, media type, and a redacted resource address.
- Supports direct URL input for a compatible media URL or player page.
- Keeps queued, running, completed, and failed downloads visible in the extension popup.
- Saves downloads locally; the extension does not upload media to an Aura server.

The free edition includes one concurrent media job and a 1 GiB limit per download. Pro adds three concurrent media jobs, no artificial byte cap, and the highest available quality for supported media.

Use this extension only for media that you own or are authorized to download. It does not bypass DRM, login restrictions, paywalls, private-video controls, or access controls. Compatibility depends on the page, media format, and browser; no universal compatibility is promised.

Support: [OWNER INPUT: SUPPORT_EMAIL]

Upgrade: [OWNER INPUT: UPGRADE_URL]

Website: [OWNER INPUT: PRODUCT_WEBSITE_URL]
