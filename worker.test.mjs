...
function postProbe(token) {
  return new Request("https://aura.mdownloader.workers.dev/api/potplayer-probe", {
    method: "POST",
    headers: { "content-type": "application/json" },
...
function probeStatus(token) {
  return new Request(
    `https://aura.mdownloader.workers.dev/api/potplayer-probe/status?token=${encodeURIComponent(token)}`,
  );
}

test("potplayer probe records a companion ping and reports it", async () => {
  const worker = await loadWorker();
  const kv = memoryKv();
  const env = environment(kv);
  const token = "a".repeat(32);
  const recorded = await worker.fetch(postProbe(token), env);
  assert.equal(recorded.status, 200);
  const seen = await worker.fetch(probeStatus(token), env);
  assert.equal((await seen.json()).seen, true);
  const missing = await worker.fetch(probeStatus("b".repeat(32)), env);
  assert.equal((await missing.json()).seen, false);
});

test("potplayer probe rejects malformed tokens", async () => {
  const worker = await loadWorker();
  const env = environment();
  assert.equal((await worker.fetch(postProbe("not-hex"), env)).status, 400);
  assert.equal((await worker.fetch(probeStatus(""), env)).status, 400);
  assert.equal((await worker.fetch(probeStatus("z".repeat(32)), env)).status, 400);
});

test("serves the PotPlayer installer from the companion bucket", async () => {
  const worker = await loadWorker();
  const bucket = {
    get: async (key) => {
      if (key !== "AuraPotPlayerSetup.exe") return null;
      return { size: 4, body: "test" };
    },
  };
  const env = environment(memoryKv(), { COMPANION_BUCKET: bucket });
  const response = await worker.fetch(
    new Request("https://aura.mdownloader.workers.dev/downloads/AuraPotPlayerSetup.exe"),
    env,
  );
  assert.equal(response.status, 200);
  assert.equal(response.headers.get("content-disposition"), 'attachment; filename="AuraPotPlayerSetup.exe"');
  assert.equal(await response.text(), "test");
  const missing = await worker.fetch(
    new Request("https://aura.mdownloader.workers.dev/downloads/AuraPotPlayerSetup.exe"),
    environment(memoryKv(), { COMPANION_BUCKET: { get: async () => null } }),
  );
